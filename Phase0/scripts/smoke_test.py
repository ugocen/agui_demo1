...
Runs the scripted S1 to S5 checks through the local FastAPI backend proxy
against the deployed AgentCore runtimes and prints a G0 report.
Prerequisites: backend running on BACKEND_URL, both runtime ARNs in .env.
...
def main() -> None:
    ...
    if AUTH_MODE == "entra" and not TOKEN:
        sys.exit("FAIL: AUTH_MODE=entra needs SMOKE_BEARER_TOKEN exported ([Human])")
    ...
    print("\n[Human] remaining G0 items:")
    print("  - Stage B rerun in the corporate account from the Windows 11 machine")
    print("  - Record the AG-UI go/no-go decision (proceed or fall back to plain SSE chat)")
