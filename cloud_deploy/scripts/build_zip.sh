#!/usr/bin/env bash
# Build an AgentCore direct-code-deployment zip for one agent (arm64 / Graviton).
# Usage: build_zip.sh <agent-dir>
# Installs deps as linux/arm64 wheels, copies the agent .py sources to the
# package root, normalizes permissions, zips from inside the package folder, and
# fails loudly on size or architecture violations. Output: <cloud_deploy>/build/<agent>.zip
# This only PACKAGES — deployment is manual via the AgentCore Console.
...
ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"   # cloud_deploy/
BUILD_DIR="$ROOT_DIR/build/$AGENT_NAME"
PACKAGE_DIR="$BUILD_DIR/package"
ZIP_PATH="$ROOT_DIR/build/$AGENT_NAME.zip"
